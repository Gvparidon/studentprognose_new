...
@memory.cache
def load_weighted_ensemble() -> Optional[pd.DataFrame]:
    logger.debug("Loading weighted ensemble dataset...")
    return _load_file(_paths['configuration']['path_ensemble_weights'], file_type="excel")
...
def load_data() -> Dict[str, Optional[pd.DataFrame]]:
    ...
    return {
        ...
        "weighted_ensemble": load_weighted_ensemble(),
